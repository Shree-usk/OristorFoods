...
- [ ] Listing supports pagination and a "related/next" recommendation at the bottom of each detail page — **partially delivered.** `listEntries`/`GET /api/food-academy` fully support `page`/`pageSize` and return `{total, page, pageSize}`, and the hub page's Zod parsing passes a `page` param through from the URL (`foodAcademyListQuerySchema`, `page.tsx:24-28`) — so `/food-academy?page=2` works if typed manually. But `src/app/(storefront)/food-academy/page.tsx` renders **no pagination UI** (no prev/next links, no page-number control) — a user has no way to reach page 2 from the hub page itself. The "related/next recommendation at the bottom of each detail page" half of this AC **is** delivered on every detail page: `findRelatedFoodAcademyEntries` (`food-academy.repository.ts`) fills same-category Published entries first and tops up with other recent Published entries when a category has no other Published siblings, so `relatedEntries` (`[slug]/page.tsx:91-100`) is never empty as long as at least one other Published entry exists anywhere. Leaving unchecked: the API/service contract and the related/next recommendation are both done, but the hub page's pagination UI is a real gap for a future story/task, not something to silently mark done.
...
- [x] **Database:**
  - [x] Define `FoodAcademyEntry` model: `id`, `slug`, `title`, `heroImageUrl`, `contentType` (enum: ARTICLE/GUIDE/COURSE), `categoryId`, `summary`, `bodyContent` (rich text/markdown for simple articles), `readingTimeMinutes`, `authorName`/`authorId`, `status`, `publishedAt`, `createdAt`, `updatedAt` — `prisma/schema.prisma:721-752`, all fields present except `authorId`: **deliberately omitted**, not forgotten — there is no admin-auth/User model in the codebase yet (STORY-038 is still Draft), so `authorName: String?` is a plain byline string for now — there is no `authorId` for entries to reference (documented in `docs/superpowers/specs/2026-09-26-food-academy-design.md` decision 2). Note: `CookingTip` has no `authorName`/`authorId` field at all, so this is not a precedent reused from `CookingTip`; it's a standalone choice driven by the same missing-User-model constraint. Enum values are `Article`/`Guide`/`Course` (PascalCase), not the story text's `ARTICLE`/`GUIDE`/`COURSE` — same naming-convention correction STORY-019 already made for `VideoProvider`.
...
- [x] **Frontend:**
  ...
  - [x] Reuse `RecipeCard` (STORY-017) and product card components (STORY-010) for the cross-link blocks rather than duplicating card markup — **checked with a partial-compliance note, not a blanket pass.** `RelatedProductsBlock` (`related-products-block.tsx`) fully reuses the real `ProductCard` component directly — fully compliant. `RelatedRecipesBlock` (`related-recipes-block.tsx`) does **not** wrap `RecipeCard`; it is a small custom presentation component. This is a deliberate design choice, not a technical constraint: `findRecipesByIds` (`recipe.repository.ts`) already selects the full `recipeCardSelect` row, and `recipe.service.ts` already has a `toRecipeCard(row)` mapper that produces exactly the shape `RecipeCard` needs — `getRecipesByIds` just currently discards that data to build the smaller `RecipePreview { id, title, slug, imageSrc }` shape instead (the same shape reused from `product-detail-extensions.ts` and already used elsewhere, e.g. the product-detail page's "Recipes using this product" block). Reusing `RecipeCard` here was possible without any extra queries; it wasn't done because the task chose to keep `RecipePreview`'s existing minimal contract consistent with its other callers, not because it would have required fabricating fake data. So: product side fully reuses `ProductCard`; recipe side uses a smaller custom component by deliberate design, not because reuse was skipped, forgotten, or blocked.
